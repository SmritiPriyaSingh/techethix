...
export const FieldNotesPreview: React.FC = () => {
  return (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {fieldCategories.map((cat, idx) => (
            <motion.div
              ...
              className="p-5 rounded-2xl bg-[#121215] border border-[#27272a] hover:border-zinc-700 transition-all group"
            >
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-sm font-bold text-white group-hover:text-emerald-400 transition-colors">{cat.name}</h3>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-zinc-900 text-zinc-400">{cat.count}</span>
              </div>
